...
void Camera::Draw()
{
    ...
    // comando de desenho
    graphics->CommandList()->DrawIndexedInstanced(indexC, 1, 0, 0, 0);
...
void Camera::BuildGeometry()
{
    ...
    while (!fin.eof())
    {
        if (lineType == 'v') {
            if (fin.get() == ' ') {
                ...
                fin >> posZ;
                if(vCounter % 7 == 0){
                    vertices[vCounter] = { XMFLOAT3(-posX, posY, posZ), XMFLOAT4(Colors::DarkRed) };   
                }
                else if(vCounter % 11 == 0) {
                    vertices[vCounter] = { XMFLOAT3(-posX, posY, posZ), XMFLOAT4(Colors::DarkOrange) };
                }else if(vCounter % 5 == 0) {
                    vertices[vCounter] = { XMFLOAT3(-posX, posY, posZ), XMFLOAT4(Colors::Orange) };
                }
                else if(vCounter % 3 == 0) {
                    vertices[vCounter] = { XMFLOAT3(-posX, posY, posZ), XMFLOAT4(Colors::OrangeRed) };
                }
                else if (vCounter % 2 == 1) {
        ...
        if (lineType == 'f')
         {
            ...
            //funciona apenas para coisa com 3 vertices
            /*temp = fin.get();
            while (temp != '\n')
            {
                temp = fin.get();
            ...
            */
            //Se comentar esse codigo abaixo e descomentar o de cima funciona
            temp = fin.get();
    ...
    const uint vbSize = vCounter * sizeof(Vertex);
    const uint ibSize = faceCounter * sizeof(ushort);
