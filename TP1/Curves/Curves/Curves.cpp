	...
	if (input->KeyPress(VK_RETURN))
	 {
		...
		if ((count * 2)-2 < MaxVertex) {
			Vertex corners[MaxVertex] = {} ;
			int cornerCount = 0;
			for (uint i = 0; i < count - 1; i++) {
				float nx = 0, ny = 0;

				nx = 3.0f / 4.0f * vertices[i].Pos.x + 1.0f / 4.0f * vertices[i + 1].Pos.x;
				ny = 3.0f / 4.0f * vertices[i].Pos.y + 1.0f / 4.0f * vertices[i + 1].Pos.y;
				corners[cornerCount++] = { XMFLOAT3(nx, ny, 0.0f), XMFLOAT4(Colors::Red) };


				nx = 1.0f / 4.0f * vertices[i].Pos.x + 3.0f / 4.0f * vertices[i + 1].Pos.x;
				ny = 1.0f / 4.0f * vertices[i].Pos.y + 3.0f / 4.0f * vertices[i + 1].Pos.y;

				corners[cornerCount++] = { XMFLOAT3(nx, ny, 0.0f), XMFLOAT4(Colors::LightGreen) };
